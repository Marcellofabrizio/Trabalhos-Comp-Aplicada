...
```{r setup, include=FALSE, out.width=100, echo = TRUE}
...
```{r beans_data, warning=FALSE, message = FALSE}
...
``` {r classes, echo = TRUE, warning=FALSE, message = FALSE}
...
``` {r echo = TRUE, fig.width=10, fig.height=10, warning=FALSE, message = FALSE}
par(mfrow=c(2,3))
for(i in 1:6){
...
Analisando os boxplots gerados, é possível notar que as classes na variável **ShapeFactor2** apresentam maior distinção entre sí, já que a área entre o 1º e 3º quartil dos seus valores está menos linhada entre os quartis das classes se comparado com as demais variáveis. Portanto, é possivel concluir que a variável **ShapeFactor2** é a que mais apresenta poder de discriminação.
...
```{r echo = TRUE, fig.width=20, warning=FALSE, message = FALSE}
library(gridExtra)
library(ggplot2)
...
p = list()
for(i in 13:15){
  p[[i-12]] = ggplot(beans_data,
                     aes_string(x=names(beans_data)[i],fill="Class")) + 
    geom_density(alpha=0.5,color="darkgray") + 
    theme(legend.position="none", 
          legend.title = element_blank())
}
...
### 7- Alguma variável (por classe) possui uma distribuição normal (curva do sino)? É possível verificar numericamente se é verdade? 
...
```{r fig.width=20, fig.height=14, warning=FALSE, message = FALSE, echo = TRUE}
...
```
A maioria dos valores estão acima da porcentagem de .05, portanto podemos presumir a normalidade dessas variáveis.
...
p[[i]] = ggplot(beans_data[beans_data$Class=="BOMBAY",], 
                aes_string(sample=names(beans_data)[i - 1]),
                color="Class")+
...
```{r fig.width=50, fig.height=50, echo = TRUE, warning=FALSE, message = FALSE}
ggpairs(beans_data, columns = 1:6, aes(colour = beans_data$Class, alpha = 0.4))
ggpairs(beans_data, columns = 7:12, aes(colour = beans_data$Class, alpha = 0.4))
ggpairs(beans_data, columns = 13:16, aes(colour = beans_data$Class, alpha = 0.4))
```
